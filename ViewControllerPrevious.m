...
// Grouped UITableView sheet for selecting the active AI model.
// ─────────────────────────────────────────────────────────────────────────────
...
// Section boundaries within the flat models array
...
- (void)viewDidLoad {
    ...
    // Do NOT register a class — we allocate subtitle cells manually in cellForRow
}
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    ...
    NSString *cap   = EZModelLabels()[model] ?: @"";
    cell.textLabel.text            = model;
    cell.textLabel.font            = [UIFont systemFontOfSize:15 weight:UIFontWeightMedium];
    cell.detailTextLabel.text      = cap;
...
// Grouped UITableView sheet for gpt-image-1 generation parameters.
// ─────────────────────────────────────────────────────────────────────────────
...
@implementation EZImageSettingsViewController {
    NSArray<NSDictionary *> *_sections;  // each dict: title, key, options[], displayNames[]
}
...
- (void)_buildSections {
    _sections = @[
        ...
        @{ @"title": @"Output Format",
           ...
           @"labels":  @[@"PNG — lossless, supports transparency",
                         @"JPEG — lossy, no transparency",
                         @"WebP — modern, supports transparency"] },
        @{ @"title": @"Background",
           @"key":   @"imgBackground",
           @"default": @"auto",
...
- (UITableViewCell *)tableView:(UITableView *)tv cellForRowAtIndexPath:(NSIndexPath *)ip {
    UITableViewCell *cell = [tv dequeueReusableCellWithIdentifier:@"ImgCell" forIndexPath:ip];
    NSDictionary *sec     = _sections[(NSUInteger)ip.section];
    NSString *val         = sec[@"options"][(NSUInteger)ip.row];
    NSString *label       = sec[@"labels"][(NSUInteger)ip.row];
    NSString *current     = [[NSUserDefaults standardUserDefaults] stringForKey:sec[@"key"]]
                            ?: sec[@"default"];
...
- (void)setupUI {
    ...
    if (@available(iOS 14.0, *)) {
        ...
        self.attachButton.menu = [UIMenu menuWithTitle:@"Attach" children:@[whisper, analyze, imageFiles, photoLib]];
...
- (void)showAttachMenu {
    // iOS 14+ contextual menu — no alert sheet needed
    if (@available(iOS 14.0, *)) {
        UIAction *whisper = [UIAction
            actionWithTitle:@"Transcribe Audio/Video (Whisper)"
                      image:[UIImage systemImageNamed:@"waveform"]
        ...
        UIMenu *menu = [UIMenu menuWithTitle:@"Attach" children:@[whisper, analyze, imageFiles, photoLib]];
        self.attachButton.menu = menu;
        self.attachButton.showsMenuAsPrimaryAction = YES;
    } else {
        // iOS 13 fallback — plain action sheet
        UIAlertController *sheet = [UIAlertController
            alertControllerWithTitle:@"Attach File" message:nil
