...
static NSString * const kTriageUncertainTurnFetchKey = @"TriageUncertainTurns";
static NSInteger kTriageUncertainTurnFetch = 3;
static NSString * const kHelperDirectAnswersEnabledKey = @"HelperDirectAnswersEnabled";

/// Reload this preference for every prompt so a change in the input control
/// takes effect on the very next triage pass.
static NSInteger _triageUncertainTurnFetchCount(void) {
    NSInteger count = [[NSUserDefaults standardUserDefaults]
        integerForKey:kTriageUncertainTurnFetchKey];
    kTriageUncertainTurnFetch = MAX(0, MIN(15, count));
    return kTriageUncertainTurnFetch;
}

/// Helper direct answers are opt-out so existing installs retain their current
/// routing behavior until the lightning-bolt setting is turned off.
static BOOL _helperDirectAnswersEnabled(void) {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    id storedValue = [defaults objectForKey:kHelperDirectAnswersEnabledKey];
    return storedValue == nil || [storedValue boolValue];
}
// ─────────────────────────────────────────────────────────────────────────────
...
NSString *EZThreadSearchMemory(NSString *searchQuery, NSString *jwtToken) {
    ...
    if (jwtToken.length == 0) {
        return loadMemoryContext(kTriageUncertainTurnFetch);
    }
...
NSArray<NSDictionary *> * _Nullable EZThreadLoadContext(NSString *threadID, NSInteger tokenBudget) {
    ...
    NSInteger recentTurnCount = MIN(kTriageUncertainTurnFetch, (NSInteger)totalTurns);
...
static NSDictionary * _Nullable _runMemoryRanker(NSString *userPrompt,
    ...
    NSString *systemPrompt =
        ...
        @"  \"selected_memories\": \"<up to 4 most relevant memory lines verbatim, newline-separated>\",\n"
        @"  \"reason\": \"<one sentence>\"\n"
        @"}";
...
void analyzePromptForContext(NSString *userPrompt,
    ...
    _triageUncertainTurnFetchCount();
    BOOL allowHelperDirectAnswers = _helperDirectAnswersEnabled();
