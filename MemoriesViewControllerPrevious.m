...
@interface EZMemoryCell : UITableViewCell <UITextViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) UILongPressGestureRecognizer *editLongPressGesture;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style
    ...
    _summaryTextView.selectable       = YES;
    ...
    _editLongPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handleSummaryLongPress:)];
    _editLongPressGesture.minimumPressDuration = 0.45;
    _editLongPressGesture.delegate = self;
    [_summaryTextView addGestureRecognizer:_editLongPressGesture];
    ...
    _editHintLabel.text            = @"✏️ Long press summary to edit";
...
- (void)handleSummaryLongPress:(UILongPressGestureRecognizer *)gesture {
    if (gesture.state != UIGestureRecognizerStateBegan) return;
    if (self.summaryTextView.isFirstResponder) return;
...
- (void)textViewDidEndEditing:(UITextView *)textView {
    ...
    card.layer.borderColor = [UIColor separatorColor].CGColor;
    card.layer.borderWidth = 1.0;
    self.editHintLabel.text  = @"✏️ Long press summary to edit";
