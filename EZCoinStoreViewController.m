...
static NSString *const kPlanUltra    = @"P-73L708182D9034800NH7EXZY";     // $20/mo 2500 coins
    ...
    @property (nonatomic, strong) NSString           *pendingOrderID;    // stored for capture after Safari returns
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest:req
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (http.statusCode == 200 && [json[@"success"] boolValue]) {
                    ...
                    [self showAlert:@"🪙 Coins Added!"
                            message:[NSString stringWithFormat:
                                @"+%ld coins added to your balance.\nNew balance: %ld coins",
                                (long)added, (long)balance]];
                    [self refreshBalance];
                    [[NSNotificationCenter defaultCenter]
                        postNotificationName:@"EZSubscriptionUpdated" object:nil];
                } else {
