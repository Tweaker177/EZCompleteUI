...
- (void)setUp
{
    ...
    //add tap gesture recogniser
    UILongPressGestureRecognizer *holdGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(didHold:)];
    holdGesture.delegate = (id <UIGestureRecognizerDelegate>)self;
    [_contentView addGestureRecognizer:holdGesture];
...
- (UIView *)containView:(UIView *)view
{
    ...
    if(view.superview == nil || view.superview != containerView)
...
- (void)didHold:(UILongPressGestureRecognizer *)holdGesture
{
    if(holdGesture.state != UIGestureRecognizerStateBegan)
        return;
    
    //check for tapped view
    NSInteger index = [self indexOfItemView:[self itemViewAtPoint:[holdGesture locationInView:_contentView]]];
    if (_delegate)
    {
        if([_delegate respondsToSelector:@selector(carousel:didHoldItemAtIndex:)])
            [_delegate carousel:self didHoldItemAtIndex:index];
    }
}
