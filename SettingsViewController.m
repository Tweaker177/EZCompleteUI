    ...
    - (void)uploadAudioForClone:(NSString *)cloneName fileURL:(NSURL *)fileURL {
       ...
       forHTTPHeaderField:@"Content-Type"];
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest:request
            ...
            // BUGFIX: Handle potential NSArray in 'detail' response to prevent unrecognized selector crash
            ...
            if ([voiceID isKindOfClass:[NSString class]] && voiceID.length > 0) {
                ...
                NSString *finalStatus = (errorMsg.length > 0) ? [NSString stringWithFormat:@"Failed: %@", errorMsg] : @"Clone creation failed.";
    ...
    - (void)showClonedVoices {
        ...
        [[[NSURLSession sharedSession] dataTaskWithRequest:request
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self updateCloneStatus:@""];
                if (cloned.count == 0) {
                    [self showAlert:@"No Cloned Voices" message:@"You haven't created any voice clones yet."];
                    return;
                }

                UIAlertController *sheet = [UIAlertController alertControllerWithTitle:@"My Cloned Voices"
                ...
                [sheet addAction:[UIAlertAction actionWithTitle:@"🗑 Delete a Clone..." style:UIAlertActionStyleDestructive handler:^(UIAlertAction *a) {
